...
Intuitively, the discrepancy between the square and the bars provides a measure of association between $X$ and $Y$. It turns out that the area of the **patches** (@fig-itspatch-2) representing that discrepancy is **equal to $\chi^2$ for unnormalized and $\frac{\chi^2}{N}$ for normalized data**. This is because for
